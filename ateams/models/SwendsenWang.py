...
from ..arithmetic import boundaryMatrix, sampleFromKernel, evaluateCochain, FINT, SparseSampleFromKernelReduced, MatrixReduction
...
class CPSwendsenWang(Model):
	...
	def __init__(self, L, temperatureFunction=constant(-0.6), initial=None, sparse=True, parallel=False, minBlockSize=32, maxBlockSize=64, cores=4):
		...
		self.SampleFromKernel = SparseSampleFromKernelReduced
	...
	def proposal(self, time):
		...
		spins = self.SampleFromKernel(self.Reducer, len(zeros))
